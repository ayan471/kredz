...
import type { EditableLoanApplication, LoanApplication, LoanApplicationData } from "@/types"
...
export async function saveLoanApplicationData(data: Record<string, string>) {
  ...
  try {
    const membershipPlan = await determineMembershipPlan(Number.parseFloat(data.monIncome))
...
export async function getLoanApplicationData(id: string): Promise<{
  ...
  try {
    ...
    if (data) {
      ...
      if (data.monIncome) {
        eligibleAmount = await calculateEligibleAmount(Number.parseFloat(data.monIncome))
      }
...
export async function updateLoanApplicationData(id: string, data: Partial<LoanApplicationData>) {
  ...
  let eligibleAmount: number | null = null
  const monthlyIncome = currentData.monIncome ? Number.parseFloat(currentData.monIncome) : 0
  const age = currentData.age || 0

  if (monthlyIncome > 0 && age > 0) {
    eligibleAmount = await getEligibleLoanAmount(age, monthlyIncome)
    console.log("Calculated eligible amount using proper logic:", eligibleAmount)
  }
...
export async function updateLoanApplication(id: string, data: EditableLoanApplication) {
  try {
    const updatedLoan = await prisma.loanApplication.update({
      ...
      data: {
        ...
        eligibleAmount: data.eligibleAmount !== undefined ? Number(data.eligibleAmount) : null,
        accountNumber: data.accountNumber,
        bankName: data.bankName,
        ifscCode: data.ifscCode,
      },
...
export async function approveLoanWithDetails(
  ...
  try {
    const updatedLoan = await prisma.loanApplication.update({
      ...
      data: {
        status: "Approved",
        approvedAmount: Number.parseFloat(data.approvedAmount),
        processingFees: Number.parseFloat(data.processingFees),
        gst: Number.parseFloat(data.gst),
        otherCharges: Number.parseFloat(data.otherCharges),
        rateOfInterest: Number.parseFloat(data.rateOfInterest),
        tenure: Number.parseInt(data.tenure),
        netDisbursement: Number.parseFloat(data.netDisbursement),
        disbursementAccount: data.disbursementAccount,
        disbursementDate: new Date(data.disbursementDate),
        lender: data.lender,
        emi: Number.parseFloat(data.emi),
      },
    })
...
export async function checkExistingLoanApplication(panNo?: string): Promise<{
  success: boolean
...
export async function updateEMIPaymentLink(loanId: string, emiPaymentLink: string) {
  try {
    const updatedLoan = await prisma.loanApplication.update({
      where: { id: loanId },
      data: { emiPaymentLink },
...
export async function toggleMembershipStatus(applicationId: string, isActive: boolean) {
  try {
    ...
    return {
      success: true,
      message: isActive ? "Membership activated successfully" : "Membership deactivated successfully",
    }
  } catch (error) {
